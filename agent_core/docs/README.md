...
ai-agent/
  ...
  │
  ├── agent_core/                           # Complete implementation
...
The agent system includes a powerful CLI for easy orchestration and integration with GitHub/GitLab workflows.
...
|---------|---------|---------|
| `agent execute` | Run all 20 agents | `agent execute task.md` |
| `agent review` | Critic review agent | `gh pr diff \| agent review` |
| `agent analyze` | Architect analysis | `agent analyze < code.py` |
| `agent status` | System health check | `agent status` |
...
**GitLab MR Review:**
```bash
glab mr diff | agent review --verbose
```

**Feature Task Execution:**
```bash
agent execute << EOF > task_result.json
...
See [tasks/EXAMPLE_REQUEST.md](agent_core/tasks/EXAMPLE_REQUEST.md) for task format.
...
| `agent_core/skills/*/SKILL.md` | 20 skill definitions |
| `agent_core/memory/*.md` | Persistent context files |
| `agent_core/orchestrator.py` | Main execution engine |
| `agent_core/config.yaml` | Configuration |
| `agent_core/tasks/*/` | Development tasks |
...
2. ✅ Run inspector: `python tools/inspector.py agent_core`
3. ✅ Try orchestrator: `python agent_core/orchestrator.py 001`
4. ✅ Create your first request in `agent_core/tasks/request_001/`
5. ✅ Monitor progress in `agent_core/memory/`
